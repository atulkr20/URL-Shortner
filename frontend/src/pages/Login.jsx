...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      showToast("CONNECTION FAILURE: SERVER UNREACHABLE", 'error');
      setLoading(false);
    }
  };
      ...
      <div style={styles.window}>
        ...
        <form onSubmit={handleLogin} style={styles.form}>
            <div style={styles.inputContainer}>
                <div style={styles.iconLabel}><Mail size={14} /><span>EML:</span></div>
                <input name="email" value={formData.email} onChange={handleChange} type="email" placeholder="OPERATOR_ID@ADDR.NET" required style={styles.input} />
            </div>

            <div style={styles.inputContainer}>
                <div style={styles.iconLabel}><Lock size={14} /><span>KEY:</span></div>
                <input name="password" value={formData.password} onChange={handleChange} type="password" placeholder="••••••••••••" required style={styles.input} />
            </div>

            <button disabled={loading} style={styles.button}>
              {loading ? <Cpu size={16} className="animate-spin" /> : <ArrowRight size={16} />}
              {loading ? "DECRYPTING..." : "INITIATE_SESSION"}
            </button>
        </form>
      </div>

      <div style={styles.footer}><span>NO CLEARANCE?</span><Link to="/signup" style={styles.link}>REQUEST IDENTITY</Link></div>
